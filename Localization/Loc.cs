...
public static class Loc
{
    ...
    private static readonly Dictionary<string, (string zh, string en)> M = new()
    {
        ...
        ["fav.exists"] = ("已經在我的最愛：{0}", "Already in favorites: {0}"),
        ["fav.title"] = ("我的最愛", "Favorites"),
        ["fav.name"] = ("名稱", "Name"),
        ["fav.rename"] = ("修改名稱", "Rename"),
        ["fav.delete"] = ("刪除", "Delete"),
        ["fav.close"] = ("關閉", "Close"),
        ["fav.deleteAsk"] = ("確定要從我的最愛刪除「{0}」？", "Remove \"{0}\" from favorites?"),
        ["fav.none"] = ("還沒有我的最愛。在要保留的分頁上，按工具列「我的最愛」→「加到我的最愛」。",
                        "No favorites yet. On a tab you want to keep, click Favorites → Add to favorites."),
        ["fav.teamBroken"] = ("這筆代理團隊的設定讀不出來，請刪除後重新加入。", "This team favorite can't be read. Delete it and add it again."),
        ...
        ["custom.detectNone"] = ("沒有偵測到可加入的工具（claude / codex / opencode / gemini / qwen 等，或都已在清單中）。",
                                  "No new tools detected (claude / codex / opencode / gemini / qwen, etc., or all already listed)."),
        ...
        ["common.ctrlEnter"] = ("Enter 換行；Ctrl+Enter 確定", "Enter for a new line; Ctrl+Enter to confirm"),
        ["common.save"] = ("儲存", "Save"),
        ...
        // 1.2.0：Multi-Agent 分頁（中文名「代理團隊」，使用者指定）
        ["ma.title"] = ("代理團隊", "Multi-Agent"),
        ["ma.menuSetup"] = ("代理團隊設定…", "Multi-Agent settings…"),
        ["ma.menuDelivery"] = ("投遞", "Delivery"),
        ["ma.menuPauseItem"] = ("暫停", "Pause"),
        ["ma.limitUnlimited"] = ("不限", "Unlimited"),
        ...
        ["ma.pickDir"] = ("選擇代理團隊的專案資料夾（所有 agent 都在這裡工作）", "Choose the Multi-Agent project folder (all agents work here)"),
        ["ma.dlgEnable"] = ("啟用", "Enable"),
        ["ma.dlgRoleNone"] = ("None", "None"),
        ["ma.dlgOpen"] = ("開啟", "Open"),
        ["ma.dlgApply"] = ("套用", "Apply"),
        ["ma.dlgCancel"] = ("取消", "Cancel"),
        ["ma.dlgRestoreRoles"] = ("還原角色檔預設", "Restore default roles"),
        ["ma.dlgOpenRoles"] = ("開啟角色檔資料夾", "Open roles folder"),
        ["ma.dlgRestoreRolesAsk"] = ("要把 common.md 與內建角色檔（PM／SE／Architect／UI/UX Designer／QA）還原成預設內容嗎？\n（你改過的內容會被覆寫；自己新增的角色檔不受影響）",
                                     "Restore common.md and the built-in role files (PM / SE / Architect / UI/UX Designer / QA) to their defaults?\n(Your edits to them are overwritten; roles you added yourself are kept.)"),
        ["ma.dlgNoBackend"] = ("這台電腦沒有找到可用的代理人類型（ClaudeCode／Codex／OpenCode／GeminiCLI）。請先安裝，或到「新連接 → 自訂…」設定路徑。",
                               "No Agent Type (ClaudeCode / Codex / OpenCode / GeminiCLI) was found on this computer. Install one, or set its path in New → Custom…."),
        ["ma.dlgHint"] = ("agent 之間用專案裡的 .ai/bus/ 資料夾互寄訊息（會自動加進 .gitignore），AwayTerminal 看到新訊息就在收件人的畫面打一行「請讀 …」。只跟下方的 Agent-x1 說話即可。\n第一次在新資料夾使用時，請先到每一格回答 CLI 的「信任此資料夾」提示，再開始下指令。",
                          "Agents send each other messages as files in the project's .ai/bus/ folder (added to .gitignore automatically); AwayTerminal types a \"read …\" line into the recipient's terminal. Talk to Agent-x1 at the bottom.\nThe first time in a new folder, answer each CLI's \"trust this folder\" prompt before giving instructions."),
        ["ma.dlgFolderMissing"] = ("資料夾不存在：\n{0}", "Folder not found:\n{0}"),
        ["ma.dlgNeedBackend"] = ("{0} 沒有選代理人類型。", "{0} has no Agent Type selected."),
        // 設定視窗每格的兩個欄位名（使用者指定：原「Coding Agent」改 Agent Type；中文「代理人類型」「代理人角色」）
        ["ma.dlgAgentType"] = ("代理人類型", "Agent Type"),
        ["ma.dlgAgentRole"] = ("代理人角色", "Agent Role"),
        ["ma.dlgRunning"] = ("執行中", "Running"),
        ["ma.dlgNotRunning"] = ("未啟用", "Not enabled"),
        ["ma.dlgNotStarted"] = ("未執行", "Not running"),
        ["ma.dlgExited"] = ("已結束", "Ended"),
        ["ma.dlgWillStart"] = (" · 套用後啟動", " · starts on Apply"),
        ["ma.dlgWillRestart"] = (" · 套用後重新啟動", " · restarts on Apply"),
        ["ma.dlgWillClose"] = (" · 套用後關閉", " · closes on Apply"),
        ["ma.dlgRestart"] = ("重新啟動", "Restart"),
        ["ma.dlgRestartOn"] = ("✓ 將重新啟動", "✓ Will restart"),
        // 既有的組按「套用」會關掉或重新啟動執行中的 agent 時先確認：{0}＝下面兩種行（每個 agent 一行）
        ["ma.applyAsk"] = ("套用後：\n{0}\n\n執行中的 agent 關閉或重新啟動後，它目前的對話就結束了。要套用嗎？",
                           "After applying:\n{0}\n\nA running agent that is closed or restarted loses its current conversation. Apply?"),
        ["ma.applyClose"] = ("・{0} 會關閉", "• {0} will be closed"),
        ["ma.applyRestart"] = ("・{0} 會重新啟動", "• {0} will be restarted"),
        // ---------- 1.2.3 AI 聊天室（New → AI聊天室；2～4 個 AI 輪流討論一個主題，主持人寫結論）----------
        ["chat.title"] = ("AI聊天室", "AI Chat Room"),
        ["chat.pickDir"] = ("選擇 AI 聊天室的資料夾（討論紀錄會寫在這裡，AI 也看得到這個專案）",
                            "Choose the AI Chat Room folder (the transcript is written here, and the AIs can read this project)"),
        ["chat.dlgRounds"] = ("討論迴數", "Rounds"),
        ["chat.dlgRoundsHint"] = ("一迴＝每個人各發言一次；討論中可以隨時按右鍵「結束討論」提前收尾",
                                  "One round = everyone speaks once; you can end early any time with right-click → End discussion"),
        ["chat.rounds"] = ("{0} 迴", "{0} rounds"),
        ["chat.dlgAiType"] = ("使用的 AI", "AI"),
        ["chat.dlgRole"] = ("角色", "Role"),
        ["chat.dlgNeedTwo"] = ("AI 聊天室至少要兩位參加者。", "An AI Chat Room needs at least two participants."),
        ["chat.dlgHint"] = ("開啟後會先問你討論主題，然後由下方的 Agent-x1（主持人）帶著大家一迴一迴發言；每個人都看得到別人已經說過的話，最後由主持人寫結論。\n討論紀錄寫在 .ai/chat/<時間>/ 底下。第一次在新資料夾使用時，請先到每一格回答 AI 的「信任此資料夾」提示。",
                            "You'll be asked for a topic, then Agent-x1 (the host) at the bottom runs the discussion round by round; everyone sees what the others have said, and the host writes the conclusion at the end.\nThe transcript is written under .ai/chat/<time>/. The first time in a new folder, answer each AI's \"trust this folder\" prompt first."),
        // 主題／插話輸入視窗
        ["chat.topicPrompt"] = ("要討論什麼？（可以寫得詳細一點，例如背景、限制、想得到什麼結論）",
                                "What should they discuss? (Feel free to add background, constraints, and what you want out of it.)"),
        ["chat.newTopicAsk"] = ("討論已經在進行中，換主題會從第 1 迴重新開始（目前的紀錄會留著）。要換嗎？",
                                "A discussion is already running. Changing the topic restarts from round 1 (the current transcript is kept). Continue?"),
        ["chat.sayPrompt"] = ("要對大家補充什麼？（會接進討論紀錄，下一位發言的人就看得到）",
                              "What do you want to add? (It goes into the transcript, so the next speaker sees it.)"),
        ...
        // xterm 搜尋列（index.html 的文字隨語言，走 T 協定）
        ...
        // 打進參加者畫面的話（{0}…＝迴數／檔案路徑）
        ["chat.turnPrompt"] = ("[AwayTerminal] 第 {0}/{1} 迴，輪到你（{2}）發言：請先讀 {3} 看大家說了什麼，再把你的發言寫進 {4}（300 字以內，UTF-8），寫完就好，不用等別人。",
        ...
        ["chat.conclusionPrompt"] = ("[AwayTerminal] 討論結束（共 {0} 迴）。請讀 {1}，整理成結論寫進 {2}（600 字以內，UTF-8），並在你的畫面上把結論顯示給使用者看。",
        ...
        ["chat.trRounds"] = ("討論迴數：{0}", "Rounds: {0}"),
        ["chat.trTurn"] = ("第 {0} 迴 · {1}（{2}）", "Round {0} · {1} ({2})"),
        ["chat.trSkipped"] = ("## 第 {0} 迴 · {1}\n\n（超過 {2} 分鐘沒有回應，跳過）\n", "## Round {0} · {1}\n\n(No reply after {2} minutes — skipped.)\n"),
        ["chat.trEnded"] = ("## 第 {0} 迴 · {1}\n\n（這一格已結束，跳過）\n", "## Round {0} · {1}\n\n(This participant has exited — skipped.)\n"),
        ...
        ["chat.stNeedTopic"] = ("等主題", "Needs topic"),
        ["chat.stConcluding"] = ("寫結論", "Concluding"),
        ["chat.stDone"] = ("已結束", "Finished"),
        ["chat.tipNeedTopic"] = ("等你給討論主題（右鍵「開始討論…」）", "Waiting for a topic (right-click → Start discussion…)"),
        ["chat.tipRound"] = ("討論中 第 {0}/{1} 迴", "Discussing — round {0}/{1}"),
        ...
        // 分頁右鍵（聊天室那一列）
        ["chat.menuSetup"] = ("AI聊天室設定…", "AI Chat Room settings…"),
        ["chat.menuTopic"] = ("開始討論／換主題…", "Start discussion / change topic…"),
        ["chat.menuSay"] = ("插話…", "Say something…"),
        ["chat.menuEnd"] = ("結束討論", "End discussion"),
        ["chat.menuOpenFolder"] = ("開啟討論紀錄資料夾", "Open transcript folder"),
        ...
        ["compose.saveFail"] = ("儲存檔案失敗", "Failed to save the file"),
        ...
        ["msg.webview2Fail"] = ("無法啟動 WebView2（終端機畫面靠它渲染）。\n請先安裝 Microsoft Edge WebView2 Runtime：\nhttps://developer.microsoft.com/microsoft-edge/webview2/\n（安裝檔版會自動安裝；免安裝 zip 版需要自行安裝）",
                                "WebView2 could not be started (the terminal is rendered with it).\nPlease install the Microsoft Edge WebView2 Runtime first:\nhttps://developer.microsoft.com/microsoft-edge/webview2/\n(The installer sets it up automatically; the portable zip does not.)"),
        ["msg.unhandled"] = ("發生未預期的錯誤（已記錄到 diag.log）。程式會繼續執行，若之後行為異常請重新啟動。",
                             "An unexpected error occurred (logged to diag.log). The app will keep running; restart it if things behave oddly."),
