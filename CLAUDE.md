...
## 待辦（需使用者輸入）
0. **⚠️ 未解：此機 PowerShell 分頁一開就死（2026-08-11 發現）**。現象＝New→PowerShell→選目錄後，`conhost.exe` 正常建立（`--headless --width 152 --height 34`，尺寸協定沒問題）、`powershell.exe` **確實有生出來但 0.5 秒內消失**，畫面只剩一個游標、打字無回應、狀態燈仍是綠的。**不是 1.0.27 造成的**——`git stash` 掉全部改動、重建 1.0.26 baseline 實測**同樣重現**，所以是環境或既有問題。最可能是 PC-cillin（本檔已記多次它對 ConPTY 開 shell 的誤判），但**未證實**。下次查法：先看防毒隔離紀錄，或用 scratchpad 的 ConPTY harness 單獨跑一次比對 exit code。註：Telnet／COM 走的是 in-process session，不受影響，可用來測不需 shell 的功能。
