...
- **New（新連接）下拉**（工具列最左）：PowerShell → SSH/Telnet → 連接埠 → **分隔線** → 你的自訂連線（ClaudeCode、opencode…）→ **ADB**（第二區最後一項）→ 分隔線 → 「自訂…」(開管理視窗)。項目為圖示+文字。
  ...
  - **代價：ADB 是內建項目，刪光自訂連線也不會消失**（使用者實際回報過這個困惑）。因此以 **設定 → ADB → 「在『新連接』選單顯示 ADB」** 勾選框控制（`AppSettings.AdbEnabled`）。註：這個旗標 1.0.16 以前就存在、字串 `settings.adbEnable` 也在，但**選單完全沒理會它、設定也沒有 UI**，等同無效欄位；1.0.17 才接上，並把預設從 `false` 改為 `true`（沿用 false 會讓全新安裝看不到 ADB）。
