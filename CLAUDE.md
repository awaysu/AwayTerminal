...
## 慣例
- **版本**：每交付一次 `<Version>` +0.0.1（「關於」顯示）。目前 **1.0.1**（1.0.0=2026-07-27 里程碑、repo 重建＋安裝檔發佈）。「關於」是自訂深色小視窗（非 MessageBox）、內容置左：標題/版本、作者行 `Awaysu (awaysu@gmail.com)` **以執行期渲染的圖片顯示**（`RenderTextImage`，依 DPI 畫、防文字收集）、Source Code 可點連結開 GitHub。
