...
- **Claude Code**：不再是內建按鈕，已**遷移成一筆自訂連線「ClaudeCode」**（`EnsureDefaults` 只做一次、以 `ClaudeMigratedToCustom` 旗標記住）。**直接以 ConPTY 執行 claude.exe（不經 PowerShell）**：claude 即主行程、一開始就用目前尺寸建立，**已移除舊的「等尺寸才送指令」hack**；勾「使用 PowerShell」或路徑是 .cmd/.bat（npm 版）時才走 PowerShell。
  - **claude 分頁輸入佇列（v1.0.28，注音修正）**：`terminal.js` 的 `sendTyped`/`qPush`/`isTypedText`。打字產生的「純文字提交」（多字元、或含非 ASCII＝IME 提交）→ 先送一個 **`ESC[I`（focus-in，claude 有開 DECSET 1004）當犧牲事件**吸收 claude 可能懸置的 ESC/Ctrl+C 待確認狀態，再**逐字、每 25ms（`PACE_MS`）**送出——與 Windows Terminal 的逐鍵行為一致。所有輸入（含貼上）走同一佇列保序（緊接的 Enter 不會超車）；單一 ASCII 按鍵/控制鍵 delay 0 即時送；claude 分頁貼上（doPaste）＝犧牲事件＋整段一次送（大量文字逐字會拖數十秒，且貼上塊由 claude 的貼上偵測處理、實測正常）。**犧牲事件不能用 NUL**——沒有懸置狀態可吸收時 NUL 會被插進輸入框變成一格隱形字元（`❯一[1C]二三` 實測）；ESC[I 沒狀態時是合法終端機事件、no-op。IME 診斷開關 `IMEDBG`（terminal.js 頂端，D 協定→C# Diag→diag.log）平時 false，追 IME 問題時改 true 重建即可看 keydown/composition/onData 全軌跡。
